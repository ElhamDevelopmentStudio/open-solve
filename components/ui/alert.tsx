...
const alertVariants = cva(
  "relative w-full rounded-none border-2 px-4 py-3 text-sm font-mono grid has-[>svg]:grid-cols-[calc(var(--spacing)*4)_1fr] grid-cols-[0_1fr] has-[>svg]:gap-x-3 gap-y-0.5 items-start [&>svg]:size-4 [&>svg]:translate-y-0.5 [&>svg]:text-current",
  {
    variants: {
      variant: {
...
function AlertDescription({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "text-muted-foreground col-start-2 grid justify-items-start gap-1 text-sm font-mono [&_p]:leading-relaxed",
        className,
      )}
      {...props}
    />

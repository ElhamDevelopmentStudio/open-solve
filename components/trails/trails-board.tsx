...
export function TrailsBoard({ problemId }: TrailsBoardProps) {
  ...
  const edges = trailsQuery.data?.edges ?? [];
  const insights = trailsQuery.data?.insights ?? [];
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <Select value={category} onValueChange={(value) => setCategory(value as TrailInsightCategory)}>
                <SelectTrigger>
          ...
          <div className="space-y-2">
            ...
            <div className="grid gap-2 md:grid-cols-2">
              {topInsights.map((insight) => (
                <label key={insight.id} className="flex cursor-pointer items-start gap-2 rounded-2xl border border-border/40 bg-muted/10 p-3 text-sm">
                  <Checkbox
                    checked={connectFrom.includes(insight.id)}
                    onCheckedChange={() =>
                      setConnectFrom((prev) =>
                        prev.includes(insight.id) ? prev.filter((id) => id !== insight.id) : [...prev, insight.id],
                      )
                    }
                  />
              ...
              {topInsights.length === 0 ? <p className="text-sm text-muted-foreground">No insights yet.</p> : null}
            </div>
          </div>
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="space-y-3 rounded-3xl border border-border/70 bg-card/70 p-6">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold uppercase text-muted-foreground">Common next steps</h3>
            <Badge variant="secondary">Graph</Badge>
          </div>
          {edges.length === 0 ? (
            <p className="text-sm text-muted-foreground">We’ll chart relationships as more insights are added.</p>
          ) : (
            <div className="space-y-2">
              {edges.map((edge) => {
                ...
                return (
                  <div key={edge.id} className="rounded-2xl border border-border/50 bg-background/80 p-3 text-sm">
